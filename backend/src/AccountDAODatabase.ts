...
export default class AccountDAODatabase implements AccountDAO, SignupAccountDAO, GetAccountAccountDAO {
	...
	async save (account: any) {
		...
		await connection.query("insert into ridedb.account (account_id, name, email, cpf, car_plate, is_passenger, is_driver) values ($1, $2, $3, $4, $5, $6, $7)", [account.accountId, account.name, account.email, account.cpf, account.carPlate, !!account.isPassenger, !!account.isDriver]);
	...
	async getByEmail (email: string) {
		const connection = pgp()("postgres://postgres:123456@localhost:5432/app");
		const [account] = await connection.query("select * from ridedb.account where email = $1", [email]);
		await connection.$pool.end();
		return account;
	}
}