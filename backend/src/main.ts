...
export async function signup (input: any): Promise<any> {
	...
	try {
		...
		const [acc] = await connection.query("select * from ridedb.account where email = $1", [input.email]);
		if (!acc) {
					...
					if (validateCpf(input.cpf)) {
						if (input.isDriver) {
							if (input.carPlate.match(/[A-Z]{3}[0-9]{4}/)) {
								await connection.query("insert into ridedb.account (account_id, name, email, cpf, car_plate, is_passenger, is_driver) values ($1, $2, $3, $4, $5, $6, $7)", [id, input.name, input.email, input.cpf, input.carPlate, !!input.isPassenger, !!input.isDriver]);
