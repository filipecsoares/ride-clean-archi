...
export default class PositionRepositoryDatabase implements PositionRepository {
	...
	async save(position: Position): Promise<void> {
		await this.connection.query("insert into ridedb.position (position_id, ride_id, lat, long, date) values ($1, $2, $3, $4, $5)", [position.positionId, position.rideId, position.coord.lat, position.coord.long, position.date]);
	}

	async listByRideId(rideId: string): Promise<Position[]> {
		const positionsData = await this.connection.query("select * from ridedb.position where ride_id = $1 order by date", [rideId]);
